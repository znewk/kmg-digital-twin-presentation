  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
         ...
         * раздел контура → промысел → осмотр значка → пауза → в начало. Пока
         * промысел был тактом прокрутки, снятия паузы хватало; теперь это
         * отдельный режим, и без него Escape оставлял бы месторождение в кадре,
         * отпустив камеру на таймлайн глобуса.
         */
        case 'Escape':
          e.preventDefault();
          if (s.entry) return;
          ...
          else if (s.tokenView) s.setTokenView(false);
