...
export function selectStoryWells(data: FieldDataset): StorySelection {
   ...
   * как разрабатываемое месторождение. На четырёх самых разнесённых кустах
   * ставятся полные модели работающих скважин: подъём флюида по колонне и
   * приток в перфорацию видны и там, за несколько километров от основного узла.
   *
   * Четыре, а не два: две точки на пятикилометровой площади всё ещё читаются
   * как пара исключений: работа идёт «вот здесь и вот здесь». С четырьмя,
   * разнесёнными максиминным отбором, добыча распределена по площади — что и
  ...
  for (const h of satelliteHubs.slice(0, 4)) {
    const w = pickNearest(
      data.wells,
      data.hubs[h].p,
      used,
