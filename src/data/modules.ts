...
export type SystemSource =
  ...
  | 'abai'
  /** Модули ABAI, которых у ЭМГ нет: подключаются в рамках пилота. */
  | 'abai-new'
...
> = {
  ...
  'abai-new': {
    label: 'ABAI',
    colorVar: 'var(--color-abai)',
    note: 'Модули ABAI, которых у ЭМГ сегодня нет: подключаются в рамках пилота',
  },
  /**
   ...
   * появляются.
   *
   * Формулировка описывает роль, а не происхождение. «Закрывают функциональные
   * разрывы» звучало как перечень недостатков действующей системы — на показе
   * это читается упрёком заказчику. Речь же о разделении труда: учётное ядро
   * остаётся за ABAI, а расчёт и моделирование добавляются поверх него.
   */
  nedra: {
    label: 'Модули ЦД',
    colorVar: 'var(--color-nedra)',
    note: 'Расчётный и модельный контур цифрового двойника поверх учётного ядра ABAI',
  },
  ext: {
    label: 'Внешние',
...
export const STATUS_META: Record<ModuleStatus, { label: string; tone: 'ok' | 'warn' | 'risk' | 'dim' }> = {
  ...
  'new-for-emg': { label: 'новое для ЭМГ · подключается пилотом', tone: 'warn' },
...
const RAW = {
  ...
  // ─── ABAI: НЕТ у ЭМГ, подключаются пилотом (ТЗ §7 п.5) ────────────────────
  abaiCrns: {
    id: 'abaiCrns',
    nameNedra: 'ABAI · ЦРНС',
...
/**
 * Список модулей без повторов ПО ОТОБРАЖАЕМОМУ ИМЕНИ.
 *
 * После перехода на единую раскладку два модуля реестра стали называться
 * одинаково: «ABAI ЦРНС 2.0» и «ABAI Управление заводнением 2.0» описаны и со
 * стороны ABAI, и со стороны контура ЦД. Это не ошибка данных — таково
 * утверждённое соответствие, и обе записи нужны, потому что у них разные
 * источник и статус. Но на экране они дают два одинаковых бейджа подряд, и
 * зритель видит в этом опечатку.
 *
 * Отсев именно по имени, а не по ключу: в раскладке вендора имена расходятся, и
 * там оба бейджа осмысленны. Правило само подстраивается под режим.
 *
 * Остаётся ПЕРВЫЙ в списке — порядок в раскадровке и разделах расставлен
 * осмысленно, и главный модуль шага стоит первым.
 */
export function dedupeModules(ids: ModuleId[], naming: NamingMode): ModuleId[] {
  const seen = new Set<string>();
  return ids.filter((id) => {
    const m = MODULES[id];
    if (!m) return false;
    const name = moduleName(m, naming);
