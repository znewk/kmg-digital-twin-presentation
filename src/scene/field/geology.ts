...
/** Полуразмеры блока недр: 1400 × 900 м. */
export const HW = 700;
export const HD = 450;
...
export function fbm(x: number, z: number): number {
  return (
    Math.sin(x * 0.006 + z * 0.009) * 0.55 +
    Math.sin(x * 0.017 - z * 0.013 + 1.7) * 0.3 +
    Math.sin(x * 0.037 + z * 0.031 + 4.2) * 0.15
  );
}

export function dome(x: number, z: number): number {
  return Math.exp(-((x * x) / (460 * 460) + (z * z) / (340 * 340)));
}
...
export function faultTraceX(depth: number, z: number): number {
  return 430 + 0.5 * (depth + 40) + 0.12 * z;
}
...
export function throwAt(x: number, z: number, depth: number): number {
  return -60 * smoothstep((x - faultTraceX(depth, z)) / 26 + 0.5);
}

type Fn = (x: number, z: number) => number;

export const ySoilTop: Fn = (x, z) => -40 + 5 * fbm(x * 0.7, z * 0.7) + throwAt(x, z, -40) * 0.5;
...
/** Площадки: рельеф выполаживается там, где стоят объекты. */
export const PADS: [number, number][] = [
  [180, -120], [420, 160], [60, 300], [300, -340], [-280, 80], [-120, -320],
  [-520, -140], [520, -220], [-650, 170], [-560, -50], [-430, -250], [-240, -390],
  [-80, -100], [100, -20], [320, -40], [620, -60], [700, -210], [240, 120],
  [-360, 220], [-140, 320], [200, -440], [120, -450],
];
...
export function surfY(x: number, z: number): number {
  ...
  for (const [px, pz] of PADS) {
    ...
    damp *= 1 - Math.exp(-d2 / (85 * 85));
  }
  return 7 * fbm(x * 0.9, z * 0.9) * damp;
}
