...
/**
 * Внутри ли точка контура снятой площади.
 *
 * Нужна всему, что не строится полярной сеткой: рельефу с его частой
 * прямоугольной сеткой и плоскостям разломов. Без этой проверки они торчали за
 * край блока прямоугольными языками — блок обрезан по контуру, а они нет.
 *
 * Оболочка выпуклая, поэтому достаточно проверить, что точка лежит по одну
 * сторону от всех её рёбер. Это дешевле трассировки луча и не спотыкается на
 * вершинах.
 */
export function insideOutline(x: number, z: number): boolean {
